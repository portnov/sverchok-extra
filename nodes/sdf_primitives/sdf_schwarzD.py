...
if sdf is None:
    add_dummy('SvExSdfSchwarzDNode', "SDF Schwarz D", 'sdf')
else:
    from sdf import *

class SvExSdfSchwarzDNode(bpy.types.Node, SverchCustomTreeNode):
    """
    Triggers: SDF Schwarz D
    Tooltip: SDF Schwarz D
    """
    bl_idname = 'SvExSdfSchwarzDNode'
    bl_label = 'SDF Schwarz D'
    ...
    def process(self):
        ...
        for params in zip_long_repeat(size_x_s, size_y_s, size_z_s, thickness_s, origins_s):
            ...
            for size_x, size_y, size_z, thickness, origin in zip_long_repeat(*params):
                sdf = schwarzD(thickness, size=(size_x,size_y,size_z),center = origin).translate(origin)
...
def register():
    if sdf is not None:
        bpy.utils.register_class(SvExSdfSchwarzDNode)

def unregister():
    if sdf is not None:
        bpy.utils.unregister_class(SvExSdfSchwarzDNode)
