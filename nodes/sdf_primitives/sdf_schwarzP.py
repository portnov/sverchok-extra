...
if sdf is None:
    add_dummy('SvExSdfSchwarzPNode', "SDF Schwarz P", 'sdf')
else:
    from sdf import *

class SvExSdfSchwarzPNode(bpy.types.Node, SverchCustomTreeNode):
    """
    Triggers: SDF Schwarz P
    Tooltip: SDF Schwarz P
    """
    bl_idname = 'SvExSdfSchwarzPNode'
    bl_label = 'SDF Schwarz P'
    ...
    def process(self):
        ...
        for params in zip_long_repeat(size_x_s, size_y_s, size_z_s, thickness_s, origins_s):
            ...
            for size_x, size_y, size_z, thickness, origin in zip_long_repeat(*params):
                sdf = schwarzP(thickness, size = (size_x,size_y,size_z),center=origin).translate(origin)
...
def register():
    if sdf is not None:
        bpy.utils.register_class(SvExSdfSchwarzPNode)

def unregister():
    if sdf is not None:
        bpy.utils.unregister_class(SvExSdfSchwarzPNode)
